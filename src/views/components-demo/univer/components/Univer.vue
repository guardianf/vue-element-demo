...
export default {
  ...
  methods: {
    ...
    init(data = {}) {
      ...
      this.api = FUniver.newAPI(univer)
      this.api.getSheetHooks().onCellDrop((cell) => {
        const { location: { row, col }, dataTransfer } = cell
        const activeSheet = this.api.getActiveWorkbook().getActiveSheet()
        const range = activeSheet.getRange(row, col, 1, 1)
        const value = dataTransfer.getData('item')
        console.log(row, col)
    ...
    handleExport() {
      const activeWorkbook = this.api.getActiveWorkbook()
      const snapshot = activeWorkbook.save()
      this.api.exportXLSXBySnapshot(snapshot).then((file) => {
        console.log(file)
